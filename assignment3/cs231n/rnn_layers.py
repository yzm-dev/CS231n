...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # Compute z and pass through tanh. 
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []  # Stores cache information for each time step
    h_states = [h0]  # Initialize the list of hidden states, starting with the initial state h0

    # Iterate over each time step in the input x
    for t in range(x.shape[1]):
        # Perform a single step forward pass of the RNN, retrieving the next hidden state and current time step cache
        next_h, cache_t = rnn_step_forward(x[:, t], h_states[t], Wx, Wh, b)
        h_states.append(next_h)  # Append the new hidden state to the list
        cache.append(cache_t)  # Append the cache to the list

    # Stack the hidden states along the time dimension, excluding the initial state h0
    h = np.stack(h_states[1:], axis=1)
